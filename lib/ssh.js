...
function _connectAndExecute(  opts ){
    ...
    c.on('ready', function() {
        ...
        if ( !!opts.sftp ){
            ...
            logger.info("executing command with execOpts :: " + JSON.stringify(opts.execOpts ));
            var execOpts = opts.execOpts || {pty:true};
            console.log("using opt :: " + JSON.stringify(execOpts));
            c.exec(opts.cmd || opts.command , execOpts, function(err, stream) {
                if (err) throw err;
